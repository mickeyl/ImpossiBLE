...
- **Single simulator app** — only one simulator app can connect to a provider at a time. While one app is connected, additional connection attempts are rejected with a busy response and the rejected client stops auto-reconnecting; the existing client keeps its scans, connections, and L2CAP channels. Multiple `CBCentralManager` instances within a single app are fully supported.
